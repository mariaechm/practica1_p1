...
function wrapCustomColumn(column, setColumnFilter, options) {
    ...
    return (_jsx(CustomColumnContext.Provider, { value: {
            setColumnFilter,
            headerFilterRenderer: headerFilterRenderer ?? NoHeaderFilter,
            filterKey: key,
        }, children: _jsx(GridColumnGroup, { header: customHeader ?? header, headerRenderer: customHeaderRenderer ?? headerRenderer, children: columnWithoutHeader }, key) }, key));
}
...
function useColumns(properties, setColumnFilter, options) {
    const sortableProperties = properties.filter((propertyInfo) => options.columnOptions?.[propertyInfo.name]?.sortable !== false);
    const [sortState, setSortState] = useState(sortableProperties.length > 0 ? { [sortableProperties[0].name]: { direction: 'asc' } } : {});
    let columns = properties.map((propertyInfo) => {
        ...
        if (!options.noHeaderFilters) {
            column = (_jsx(GridColumnGroup, { headerRenderer: HeaderSorter, children: _jsx(GridColumn, { path: propertyInfo.name, headerRenderer: HeaderFilterWrapper, ...columnProps }) }));
        }
        else {
            column = _jsx(GridColumn, { path: propertyInfo.name, headerRenderer: HeaderSorter, ...columnProps });
        }
        return (_jsx(ColumnContext.Provider, { value: {
                propertyInfo,
                setColumnFilter,
                sortState,
                setSortState,
    ...
    if (options.rowNumbers) {
        columns = [
            _jsx(GridColumn, { width: "4em", flexGrow: 0, renderer: AutoGridRowNumberRenderer }, "rownumbers"),
            ...columns,
        ];
    }
    const { totalCount, filteredCount, itemCounts, footerCountRenderer } = options;
    if (totalCount ?? filteredCount) {
        const col = (_jsx(FooterContext.Provider, { value: {
                totalCount,
                filteredCount,
                footerCountRenderer,
                itemCounts,
            }, children: _jsx(GridColumnGroup, { footerRenderer: AutoGridFooterItemCountRenderer, children: columns }) }, "grid-footer"));
        columns = [col];
    }
    return columns;
}
function AutoGridInner({ service, model, itemIdProperty, experimentalFilter, visibleColumns, hiddenColumns, noHeaderFilters, customColumns, columnOptions, rowNumbers, totalCount, filteredCount, footerCountRenderer, ...gridProps }, ref) {
    ...
    const dataProviderRef = useRef(undefined);
    ...
    const setHeaderFilter = (filter, filterKey) => {
        ...
        const indexOfFilter = filterKey
            ? internalFilter.children.findIndex((f) => f.key === filterKey)
            : -1;
    ...
    return (_jsx(Grid, { itemIdPath: modelInfo.idProperty?.name, ...gridProps, ref: gridRef, children: children }));
}
export const AutoGrid = forwardRef(AutoGridInner);
