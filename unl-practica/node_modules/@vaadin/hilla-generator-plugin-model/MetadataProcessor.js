...
function createAnnotationsProperty(schema) {
    ...
    const annotationLiterals = annotations.map((annotation) => ts.factory.createObjectLiteralExpression([
        ts.factory.createPropertyAssignment('name', ts.factory.createStringLiteral(annotation.name)),
    ]));
    return ts.factory.createPropertyAssignment('annotations', ts.factory.createArrayLiteralExpression(annotationLiterals));
}
...
export function process(schema) {
    const schemaWithMetadata = schema;
    const properties = [createAnnotationsProperty(schemaWithMetadata), createJavaTypeProperty(schemaWithMetadata)].filter(Boolean);
