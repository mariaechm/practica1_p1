...
import { convertFullyQualifiedNameToRelativePath, convertReferenceSchemaToPath, convertReferenceSchemaToSpecifier, decomposeSchema, isComposedSchema, isEnumSchema, isObjectSchema, isReferenceSchema, simplifyFullyQualifiedName, } from '@vaadin/hilla-generator-core/Schema.js';
...
export class EntityModelProcessor {
    ...
    process() {
        ...
        return createSourceFile([...importStatements, declaration, ...exportStatement].filter(Boolean), this.#outputPathManager.createRelativePath(this[$model].path));
    }
}
export class EntityClassModelProcessor extends EntityModelProcessor {
    ...
    constructor(name, component, context) {
        ...
        this.#makeObjectEmptyValueCreator = this[$dependencies].imports.named.add('@vaadin/hilla-lit-form', 'makeObjectEmptyValueCreator');
    }
    [$processDeclaration]() {
        ...
        if (isComposedSchema(this.#component)) {
            ...
            if (decomposed.length > 2) {
                logger.debug(this.#component, `The schema for a class component ${this.#fullyQualifiedName} has more than two components. This plugin will ignore it.`);
                return undefined;
            }
    ...
    #processClassElements({ properties }) {
        ...
        return Object.entries(properties).map(([name, schema]) => {
            ...
            return ts.factory.createGetAccessorDeclaration(undefined, ts.factory.createIdentifier(name), [], type, ts.factory.createBlock([
                ts.factory.createReturnStatement(ts.factory.createCallExpression(ts.factory.createElementAccessExpression(ts.factory.createThis(), this.#getPropertyModelSymbol), undefined, [ts.factory.createStringLiteral(name), createModelBuildingCallback(type.typeName, args)])),
            ], true));
        });
    }
    #processModelClass(schema, entity, parent) {
        ...
        return ts.factory.createClassDeclaration(undefined, this[$model].id, [modelTypeParameters], [
...
export class EntityEnumModelProcessor extends EntityModelProcessor {
    ...
    [$processDeclaration]() {
        ...
        const makeEnumEmptyValueCreator = this[$dependencies].imports.named.add('@vaadin/hilla-lit-form', 'makeEnumEmptyValueCreator');
        return ts.factory.createClassDeclaration(undefined, this[$model].id, undefined, [
            ts.factory.createHeritageClause(ts.SyntaxKind.ExtendsKeyword, [
            ...
            ts.factory.createPropertyDeclaration([ts.factory.createModifier(ts.SyntaxKind.ReadonlyKeyword)], ts.factory.createComputedPropertyName(enumPropertySymbol), undefined, undefined, this[$entity].id),
        ]);
    }
}
